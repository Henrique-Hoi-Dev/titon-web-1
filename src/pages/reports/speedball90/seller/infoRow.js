...
const InfoRow = (props) => {
  ...
  const { data, setSellerId, setGameInstanceId, setShowModalUpdateSeller, index } = props;
  ...
  return (
    <>
      <SRow key={data.id} alternatingcolors={index}>
        ...
        <SCell displaywidth={isSmallMobile ? 1 : 0}>{data.fee}%</SCell>
        <SCell displaywidth={isMobile ? 1 : 0}>{data.pin}</SCell>
        <SCell displaywidth={isMobile ? 1 : 0}>{data.room_name}</SCell>
        <SCell displaywidth={isDesktop ? 1 : 0}>{data.salespoint_name}</SCell>
