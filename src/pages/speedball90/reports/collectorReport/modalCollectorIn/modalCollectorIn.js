...
const ModalCollectorIn = (
  ...
  const debouncedCollectorId = useDebounce(id);
  ...
  const INITIAL_STATE_QUERY = {
    ...
    end_date: endDate,
    collector_id: debouncedCollectorId,
    game_instance: debouncedGameInstanceId
  };
  const [query, setQuery] = useState(INITIAL_STATE_QUERY);

  const { data } = useGetBingo("bingo-reports/collector/report/in", query, id ? false : true);
...
}, [debouncedGameInstanceId, debouncedCollectorId, startDate, endDate]);
  ...
  return (
    ...
    >
      <ContentHeader>
        <Title>{t("field.contribution")} </Title>
        </ContentHeader>
          ...
          <STable>
            <SHead>
              <SRow>
                ...
                <SCell>
                  <SLabel
                    active={query?.sort_field === "entry_id"}
...
export default ModalCollectorIn;
