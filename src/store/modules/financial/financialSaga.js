...
function* getFinancialById({ payload }) {
  try {
    const response = yield call(api.get, `manager/financialStatement/${payload}`)
...
function* createFinancial({ payload }) {
  try {
    const response = yield call(api.post, 'manager/financialStatement', payload)
...
function* updateFinancial({ payload }) {
  try {
    ...
    const response = yield call(api.patch, `manager/financialStatement/${id}`, data)
...
function* finishingFinancial({ payload }) {
  try {
    const { id, data } = payload
    const response = yield call(api.patch, `manager/financialStatement/finishing/${id}`, data)
