...
const BaseModalDeleteTruck = ({ showModal, setShowModal, props }) => {
  ...
  const handleSubmit = (ev) => {
    ev.preventDefault();
    dispatch(deleteTruckRequest(props.id));
  };
  ...
  return (
    ...
    >
      <ContentHeader>
        <Title>{t('modal_truck.title_delete')}</Title>
      </ContentHeader>
        ...
        <Grid container item spacing={2}>
          ...
          >
            <Grid item xs={12} md={12} lg={12}>
              <p>{t('modal_truck.description_delete')}</p>
            </Grid>
          </Grid>
