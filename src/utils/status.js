...
export const typeStatus = (res) => {
  ...
  const firstStatusProps = status.find((item) => item.value === firstStatus?.status) ?? ''

  const secondStatus = res.find((item) => item.status === 'PENDING') ?? ''
  const secondStatusProps = status.find((item) => item.value === secondStatus?.status) ?? ''

  const thirdStatus = res.find((item) => item.status === 'APPROVED') ?? ''
  const thirdStatusProps = status.find((item) => item.value === thirdStatus?.status) ?? ''

  const fourthStatus = res.find((item) => item.status === 'DENIED') ?? ''
  const fourthStatusProps = status.find((item) => item.value === fourthStatus?.status) ?? ''

  const fifthStatus = res.find((item) => item.status === 'FINISHED') ?? ''
  const fifthStatusProps = status.find((item) => item.value === fifthStatus?.status) ?? ''

  const fifthStatus1 = res.find((item) => item.status === 'DRAFT') ?? ''
  const fifthStatusProps1 = status.find((item) => item.value === fifthStatus1?.status) ?? ''
